...
const YouTubeMediaSuite = () => {
  ...
  const [enableCookieHeader, setEnableCookieHeader] = useState(false);

  useEffect(() => {
    try {
      const savedCookie = localStorage.getItem('yt_cookie_header') || '';
      const savedEnable = localStorage.getItem('yt_cookie_enable') === 'true';
      setCookieHeader(savedCookie);
      setEnableCookieHeader(savedEnable);
    } catch (e) {
  ...
  const saveCookieSettings = () => {
    try {
      localStorage.setItem('yt_cookie_header', cookieHeader || '');
      localStorage.setItem('yt_cookie_enable', enableCookieHeader ? 'true' : 'false');
  ...
  const buildAuthHeaders = () => {
    const headers = {};
    if (enableCookieHeader && cookieHeader && cookieHeader.trim().length > 0) {
      headers['X-YouTube-Cookie'] = cookieHeader.trim();
    }
    return headers;
  };
        ...
        {activeTab === 'settings' && (
          <div className="space-y-4">
            ...
            <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl p-6 shadow-lg space-y-6`}>
              ...
              <div>
                <h3 className="font-semibold mb-3">YouTube Cookie Header (Bot Check Bypass)</h3>
                <p className={`text-sm mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  Paste the Cookie header from an authenticated YouTube request. This helps when YouTube requires sign-in to confirm you're not a bot.
                </p>
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm">Use Cookie header for requests</span>
                    <input
                      type="checkbox"
                      className="w-5 h-5"
                      checked={enableCookieHeader}
                      onChange={(e) => setEnableCookieHeader(e.target.checked)}
                    />
                  </div>
                  <textarea
                    value={cookieHeader}
                    onChange={(e) => setCookieHeader(e.target.value)}
                    placeholder="VISITOR_INFO1_LIVE=...; YSC=...; __Secure-...=...; ..."
                    rows={4}
                    className={`w-full px-4 py-2 rounded-lg ${
                      darkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-100 border-gray-300'
                    } border`}
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-4 py-2 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-lg font-semibold hover:from-red-600 hover:to-pink-600"
                    >
                      Save Cookie Settings
                    </button>
                    <button
                      onClick={() => { setCookieHeader(''); setEnableCookieHeader(false); saveCookieSettings(); }}
                      className={`${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-200 hover:bg-gray-300'} px-4 py-2 rounded-lg`}
                    >
                      Clear
                    </button>
                  </div>
                  <p className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                    How to get it: Open YouTube, sign in, press F12 → Network, reload page, select a request to youtube.com, copy the Request Headers value for "cookie:" (without the word "cookie:").
                  </p>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-gray-700">
                <button
                  onClick={saveCookieSettings}
                  className="w-full py-3 bg-gradient-to-r from-red-500 to-pink-500 text-white rounded-lg font-semibold hover:from-red-600 hover:to-pink-600"
                >
                  Save Settings
                </button>
              </div>
